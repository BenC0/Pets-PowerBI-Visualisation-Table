...
export class Visual implements IVisual {
        ...
        for (let row_index = 0; row_index < table_shape.rows; row_index++) {
            ...
            for (let column_index = 0; column_index < table_shape.columns; column_index++) {
                ...
                if (!!formatString && typeof value !== "string") {
                    formatted_value = this.applyCustomFormat(formatted_value, formatString);
                }
                row_html += `<td
                    dimension=${!!column.roles.dimension}
                    metric=${!!column.roles.metrics}
    ...
    public applyCustomFormat(number: number, formatString: string) {
        ...
        formattedValue = number.toLocaleString(undefined, this.parseFormatString(chosenFormat));

        return formattedValue;
    }

    public parseFormatString(formatString: string) {
